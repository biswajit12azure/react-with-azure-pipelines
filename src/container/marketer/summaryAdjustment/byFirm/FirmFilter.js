...
import { alertActions, firmAction, marketergroupAction } from '_store';
...
const FirmFilter = ({ handleFilterSubmit, selectedDate, setSelectedDate, isOpen, onClose, onOpen }) => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const result = await dispatch(firmAction.get(transformedData)).unwrap();
            if (result?.error) {
                        ...
                        <Typography component="div" className='userprofilelist'>
                            <Grid container direction="row" spacing={{ xs: 2, md: 3 }}>
                                <Grid item xs={12} sm={6} md={6}>
                                    <Typography variant="h2" className='userprofilelistcontent'>By Firm</Typography>
                                </Grid>
                            </Grid>
                        </Typography>
