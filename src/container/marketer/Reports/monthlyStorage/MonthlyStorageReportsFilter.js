...
import { alertActions, monthlyStorageAction } from "_store";
...
const MonthlyStorageFilter = ({
  ...
  const marketerList =
    marketerData?.MarketerList?.map((x) => ({
      label: x.Description,
      value: x.CompanyID,
    })) || [];
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const result = await dispatch(
        monthlyStorageAction.get(transformedData)
      ).unwrap();
      if (result?.error) {
        dispatch(
          alertActions.error({
...
export default MonthlyStorageFilter;
