...
const DCReportsFilter = ({ handleFilterSubmit, isOpen, onClose, onOpen,marketers }) => {
    const dispatch = useDispatch();
    const [anchorEl, setAnchorEl] = useState(null);
    const [isFormValid, setIsFormValid] = useState(false);


    const marketerList = marketers?.map(x => ({
            ...
            <Popper id={id} open={canBeOpen} anchorEl={anchorEl} className="Filtercontainer">
                <Box sx={{ border: 1, p: 1, bgcolor: 'background.paper' }} className="Filtercontainerinner Border">
                    <form onSubmit={handleSubmit(onSubmit)} className='Registrationcontainer marketerFiltercontainer'>
                        ...
                        />
                        <Typography component="div" className=" selecticon">
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <Controller
                                ...
                                render={({ field }) => (
                                    <DatePicker
                                        className='SelectedDate widthfilter '
                                        {...field}
                                        label="Select Month"
                                        views={['year', 'month']}
...
export default DCReportsFilter;