...
import { activityInterruptibleAction, alertActions } from "_store";
...
import { monthlyStorageSchema } from "_utils/validationSchema";
import { AutocompleteInput } from "_components";

const ActivateActivityInterruptibleFilter = ({
  ...
  const marketerList =
  activityInterruptible?.Marketers?.map((x) => ({
  ...
  const {
    ...
    resolver: yupResolver(monthlyStorageSchema),
    defaultValues: {
      ReportDate: date,
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const result = await dispatch(
        activityInterruptibleAction.get(transformedData)
      ).unwrap();
      if (result?.error) {
        dispatch(
          alertActions.error({
...
export default ActivateActivityInterruptibleFilter;
