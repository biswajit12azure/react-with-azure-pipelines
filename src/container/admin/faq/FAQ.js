...
const FAQ = () => {
  ...
  const adminList = user?.UserAccess?.filter(
    (access) => access.Role.toLowerCase() === "admin"
  );

  const portalsList = adminList
    ? adminList.map((admin) => ({
    ...
    const fetchData = async () => {
      try {
        ...
        const faq = await dispatch(
          faqAction.getByAdminRole(authUserId)
        ).unwrap();
        setFaqData(faq?.Data);
      } catch (error) {
        dispatch(
  ...
  return (
    <Box className="Announcementcontainerlist">
      <Typography component="h2" className="Announcementcontent">
        FAQ MANAGEMENT
      </Typography>
      <form onSubmit={handleSubmit(onSubmit)} className="Registrationcontainer">
        <Grid container direction="row" spacing={{ xs: 2, md: 3 }}>
          <Grid
            ...
            className="CardDetailContainer-right"
          >
              ...
              <Grid size={{ xs: 12, sm: 12, md: 6 }}>
                <Grid container>
                  <Grid size={{ xs: 12, sm: 12, md: 6 }}>
                    <Typography
                      component="div"
                      className="passwordcheck marbottom0 selecticon"
                    >
                      <AutocompleteInput
                        id="PortalID"
                        control={control}
